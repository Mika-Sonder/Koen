...
const novelSorts = [{ value: "Relevance desc", label: "Popularidad" }, { value: "Start date desc", label: "Más recientes" }, { value: "Start date asc", label: "Más antiguas" }, { value: "Title asc", label: "Título A–Z" }, { value: "Num. books desc", label: "Más volúmenes" }];
...
export function ExploreFilters({ fixedType, basePath = "/explorar" }: { fixedType?: "ANIME" | "MANGA" | "NOVEL"; basePath?: string } = {}) {
  ...
  return <div className="glass mb-8 flex flex-wrap items-center gap-3 rounded-2xl p-3">
    ...
    {type === "NOVEL" && <select className={selectClass} value={current.get("orden") ?? "Relevance desc"} onChange={(event) => set("orden", event.target.value)} aria-label="Orden">{novelSorts.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select>}
